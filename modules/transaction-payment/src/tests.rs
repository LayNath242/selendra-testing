...
use super::*;
use crate::mock::{AlternativeFeeSurplus, KUSDFeeSwapPath, CustomFeeSurplus, DotFeeSwapPath};
use frame_support::{
...
use mock::{
	...
	System, TransactionPayment, SEL, ALICE, KUSD, BOB, CHARLIE, DAVE, DOT, FEE_UNBALANCED_AMOUNT,
...
fn charges_fee_when_validate_and_native_is_not_enough() {
	...
	builder_with_dex_and_fee_pool(true).execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(SEL, &BOB), ed);
		assert_eq!(
			Currencies::free_balance(KUSD, &BOB),
...
fn charges_fee_failed_by_slippage_limit() {
	builder_with_dex_and_fee_pool(true).execute_with(|| {
		...
		assert_eq!(
			DEXModule::get_swap_amount(&vec![KUSD, SEL], SwapLimit::ExactTarget(Balance::MAX, 2010)),
			Some((252, 2010))
		);
		assert_eq!(
...
fn set_alternative_fee_swap_path_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TransactionPayment::set_alternative_fee_swap_path(Origin::signed(ALICE), Some(vec![KUSD, DOT])),
				Error::<Runtime>::InvalidSwapPath
			);
...
fn swap_from_pool_with_enough_balance() {
	builder_with_dex_and_fee_pool(true).execute_with(|| {
		...
		assert_eq!(
			expect_treasury_kusd,
			Currencies::free_balance(KUSD, &usd_fee_account) - usd_ed
		);
		assert_eq!(expect_user_sel, Currencies::free_balance(SEL, &BOB));
...
fn charge_fee_failed_when_disable_dex() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			kusd_ed + (210 + surplus) * 10,
			Currencies::free_balance(KUSD, &fee_account)
		);
...
fn charge_fee_pool_operation_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let treasury_account: AccountId = <Runtime as Config>::TreasuryAccount::get();
		let sub_account: AccountId = <Runtime as Config>::PalletId::get().into_sub_account_truncating(KUSD);
		let usd_ed = <Currencies as MultiCurrency<AccountId>>::minimum_balance(KUSD);
		let pool_size = FeePoolSize::get();
		...
		let kusd_amount1 = <Currencies as MultiCurrency<AccountId>>::free_balance(KUSD, &sub_account);
		...
		let kusd_amount2 = <Currencies as MultiCurrency<AccountId>>::free_balance(KUSD, &sub_account);
